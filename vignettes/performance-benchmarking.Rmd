...
This vignette shows the relative filtering time of the hard_filter() function from SNPfiltR versus the same filtering approach using VCFtools. I repeated this procedure and found little difference between compressed (.gz) vcf input and standard vcf input, so the comparisons shown here are done with gzipped vcf files as input. All of the input vcf files necessary to fully reproduce this performance comparison can be found [here](github.com/DevonDeRaad/SNPfiltR.benchmarking.data)
